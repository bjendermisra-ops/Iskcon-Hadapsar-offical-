 ...
 * Core Logic: Three.js Sacred Scene + GSAP + Lenis + Cursor + Tilt + Magnetic
 * © 2026 ISKCON Hadapsar. All Rights Reserved.
 *
 * NOTE: Pro-optimized with crash-proof initializers and bulletproof mobile scroll logic.
 */
...
window.addEventListener('load', function () {
  ...
  var _interval = setInterval(function () {
    ...
    if (progress >= 100) {
      ...
      // Add a slight delay for aesthetic completion before hiding
      setTimeout(function () {
        if (loader) {
  ...
  /* ═══════════════════════════════════════════════════════════
     MASTER INIT (With Crash-Proof Try/Catch)
  ═══════════════════════════════════════════════════════════ */
  function _initAll() {
    try { _initLenis(); } catch(e) { console.warn("Lenis init bypassed", e); }
    try { _initThree(); } catch(e) { console.warn("Three.js init bypassed", e); }
    try { _initGSAP(); }  catch(e) { console.warn("GSAP init bypassed", e); }
    ...
    try { _initParallaxOrbs(); } catch(e) {}
  }

  /* ═══════════════════════════════════════════════════════════
     2. LENIS SMOOTH SCROLL (BULLET-PROOF IMAGE SCROLL FIX)
  ═══════════════════════════════════════════════════════════ */
  function _initLenis() {
    ...
    var lenis = new Lenis({
      ...
      smoothWheel:     true,
      smoothTouch:     false, /* Critical: Must be false for native mobile swipe */
      wheelMultiplier: 1,
      touchMultiplier: 1.5,
      /* 
       * CRITICAL FIX: Force Lenis to ignore touches on images and scroll galleries.
       * This allows native browser scrolling when the user swipes over an image.
       */
      prevent: function(node) {
        return node.nodeName === 'IMG' || 
               node.classList.contains('scroll-gallery') || 
               node.classList.contains('app-mockup') ||
  ...
  function _initThree() {
    ...
    // Prevent canvas from stealing pointer events just in case
    ...
    /* ── CORE FIX: FORCE TRANSPARENT BACKGROUND FOR WebGL ── */
    ...
    /* ── TORUS KNOT (Sacred Geometry — premium accent for desktop) ── */
    var knotMesh;
    ...
    for (var i = 0; i < pCount; i++) {
      ...
      var t = Math.random();
      if (t < 0.45) {
        pColors[i3] = 1.0; pColors[i3+1] = 0.87; pColors[i3+2] = 0.1;   
      } else if (t < 0.75) {
        pColors[i3] = 1.0; pColors[i3+1] = 0.52; pColors[i3+2] = 0.0;   
      } else {
        pColors[i3] = 0.88; pColors[i3+1] = 0.88; pColors[i3+2] = 1.0;  
      }
    }
    ...
    /* ── GSAP SCROLL BINDINGS for 3D elements ── */
  ...
  function _initGSAP() {
    ...
    if (document.querySelector('.about-img-main img')) {
    ...
    if (document.querySelector('.gs-hero-left')) {
    ...
    if (document.querySelector('.gs-hero-right')) {
  ...
  /* ═══════════════════════════════════════════════════════════
     5. CUSTOM CURSOR (Disabled on Touch Devices)
  ═══════════════════════════════════════════════════════════ */
  ...
  function _initCounters() {
    ...
    var obs = new IntersectionObserver(function (entries) {
      entries.forEach(function (en) {
        ...
        var startT  = null;
        var dur     = 1800; // 1.8 seconds animation
  ...
  function _initParallaxOrbs() {
    ...
    if (g1) gsap.to(g1, { y: '-25%', ease: 'none', scrollTrigger: { trigger: 'body', start: 'top top', end: 'bottom bottom', scrub: 2 } });
    if (g2) gsap.to(g2, { y: '-18%', x: '8%', ease: 'none', scrollTrigger: { trigger: 'body', start: 'top top', end: 'bottom bottom', scrub: 3 } });
    if (g3) gsap.to(g3, { y: '-30%', x: '-5%', ease: 'none', scrollTrigger: { trigger: 'body', start: 'top top', end: 'bottom bottom', scrub: 2.5 } });
  }
